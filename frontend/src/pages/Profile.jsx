...
const Profile = () => {
  ...
  const [stats, setStats] = useState({
    totalTests: 0,
    avgWpm: 0,        // Changed from averageWpm to match backend
    avgAccuracy: 0,   // Changed from averageAccuracy to match backend
    bestWpm: 0
  });
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchUserStats = async () => {
      try {
        // Fixed typo in endpoint: "states" -> "stats"
        const res = await axios.get('http://localhost:5000/api/results/stats', {
          headers: { Authorization: `Bearer ${user.token}` }
        });
        
        // Make sure we're accessing the stats property from the response
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
      ...
      <div className="mb-6 flex items-center">
        ...
        <div>
          <p className="text-xl font-semibold">{user?.name || user?.username || 'User'}</p>
          <p className="text-gray-600">{user?.email}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-gray-50 p-3 rounded">
            <p className="text-gray-600 text-sm">Avg. WPM</p>
            <p className="text-xl font-bold">{(stats.avgWpm || 0).toFixed(1)}</p>
          </div>
          <div className="bg-gray-50 p-3 rounded">
            <p className="text-gray-600 text-sm">Avg. Accuracy</p>
            <p className="text-xl font-bold">{(stats.avgAccuracy || 0).toFixed(1)}%</p>
          </div>
          <div className="bg-gray-50 p-3 rounded">
            <p className="text-gray-600 text-sm">Best WPM</p>
            <p className="text-xl font-bold">{stats.bestWpm}</p>
